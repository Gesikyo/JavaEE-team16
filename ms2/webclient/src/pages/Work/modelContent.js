  ...
  return (
    <Form {...formItemLayout} autoComplete="off" form={form} onFinish={onFinish}>
      ...
      <Form.Item label="月份" name="workmouth">
        {readOnly? <span>{editItem?.workmouth}</span> :<Input type='number' suffix="月" allowClear min={1} max={12}/>}
      </Form.Item>
      <Form.Item label="年份" name="workyear">
        {readOnly? <span>{editItem?.workyear}</span> :<Input type='number' suffix="年" allowClear min={1946} />}
      </Form.Item>
