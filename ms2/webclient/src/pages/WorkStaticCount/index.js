...
const Page = () => {
  ...
  const onSummary = (pageData) => {
    ...
    pageData.forEach(({ unFinish, progress, finished,unknow }) => {
      total1+=unFinish;
      total2+=progress;
      total3+=finished;
      total4+=unknow;
    ...
    return (
      <>
        <Table.Summary.Row>
          ...
          <Table.Summary.Cell index={5} align='center'>{total4}</Table.Summary.Cell>
        </Table.Summary.Row>
      </>
    );
  }
